...
func (hm *HostMap) addRemoteIndexHostInfo(index uint32, h *HostInfo) {
	...
	hm.Unlock()
	l.Sugar().Debug("Hostmap remoteIndex added", "hostMap", m{"mapName": hm.name, "indexNumber": index, "mapTotalSize": len(hm.Indexes),
...
func (hm *HostMap) DeleteHostInfo(hostinfo *HostInfo) {
	...
	l.Sugar().Debug("hostmap hostinfo delete", "hostMap", m{"mapName": hm.name, "mapTotalSize": len(hm.Hosts),
